...
void ADualityCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindTouch(IE_Pressed, this, &ADualityCharacter::TouchStarted);
	PlayerInputComponent->BindTouch(IE_Released, this, &ADualityCharacter::TouchStopped);
...
{
	// If Duality is added to a project via 'Add Feature' in the Unreal Editor the dependency on HeadMountedDisplay in Duality.Build.cs is not automatically propagated
...
void ADualityCharacter::TouchStarted(ETouchIndex::Type FingerIndex, FVector Location)
{
		Jump();
}

void ADualityCharacter::TouchStopped(ETouchIndex::Type FingerIndex, FVector Location)
{
		StopJumping();
}
